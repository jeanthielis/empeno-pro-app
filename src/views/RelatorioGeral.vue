                ...
                <table class="w-full text-sm text-left">
                  ...
                  <tbody class="divide-y divide-gray-100 dark:divide-slate-800">
                    <tr v-for="a in dadosFiltrados.atrito" :key="a.id" class="hover:bg-teal-50/30 dark:hover:bg-teal-900/5">
                      ...
                      <td class="p-3"><div class="text-gray-700 dark:text-gray-300 text-xs">{{ a.produto }}<span v-if="a.formato" class="text-gray-400"> · {{ a.formato }}</span></div><span class="font-mono font-black text-xs bg-gray-100 dark:bg-slate-800 px-1.5 py-0.5 rounded mt-0.5 inline-block">{{ a.lote }}</span></td>
...
const camposBase  = computed(() => {
  ...
  if (modalTipo.value==='atrito') {
    c.splice(3,0,{label:'Classe AD',valor:i.classeAD})
    if (i.formato) c.splice(3,0,{label:'Formato',valor:i.formato})
  }
...
const exportarExcelGeral = () => {
  ...
  if(ncAtrito.value.length){const ws=XLSX.utils.json_to_sheet(ncAtrito.value.map(a=>({'Data':fmtData(a),'Hora':fmtHora(a),'Inspetor':fv(a.inspetor),'Linha':fv(a.linha),'Produto':fv(a.produto),'Formato':fv(a.formato),'Lote':fv(a.lote),'Classe AD':fv(a.classeAD),'Média':a.media?.toFixed(3)??''  })));XLSX.utils.book_append_sheet(wb,ws,'Atrito')}
